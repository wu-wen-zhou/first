...
#         self.multiheadAttention = torch.nn.MultiheadAttention(512,8)
#         self.layernorm = torch.nn.LayerNorm(512)
#         self.contrastive_temperature = 0.02
#         self.use_dual_ctr = False
...
#         out = decoder_output[1]["encoder_out"]
#         mix_encoder_out,encoder_padding_mask = out["encoder_out"][0],out["encoder_padding_mask"][0]
#         loss, _ = self.compute_loss(model, decoder_output, sample, reduce=reduce)
#         return loss, decoder_output,mix_encoder_out.transpose(0,1),encoder_padding_mask
...
#         if self.training:#st_output和mt_output是解码器的输出  audio_encpder_out相当于直接是tensor了不是字典格式，st_enc_out是字典，里面包含audio_encoder_out
...
#             st_lprobs, target_audio, st_probs = self.get_lprobs_and_target(model, st_output, sample)
#             mt_lprobs, target_text, mt_probs = self.get_lprobs_and_target(model, mt_output, sample)
#             # cross_loss = (st_loss / cross_loss_st_mt) * st_loss + (mt_loss / cross_loss_st_mt) * mt_loss
#             #自注意力机制
...
#                 # attention,_ = self.multiheadAttention(audio_encoder_out,text_encoder_out,text_encoder_out)
#                 # mixup = self.get_mixed_sequence(x=audio_encoder_out,y=attention,ot=ot_st,p=self.mix_prob)
#                 # mixup = audio_encoder_out + mixup

#                 mixup = self.get_mixed_sequence(x=audio_encoder_out, y=text_encoder_out, ot=ot_st, p=self.mix_prob) # [B, T, C]
#                 residual = mixup
#                 # attention,_ = self.multiheadAttention(audio_encoder_out,mixup,mixup)
#                 # text_encoder_out1 = F.interpolate(text_encoder_out.transpose(1,2), size=audio_encoder_out.size(1), mode='nearest').transpose(1,2)
#                 # attention,_ = self.multiheadAttention(text_encoder_out1,mixup,mixup)
#                 # attention,_  = self.multiheadAttention(mixup,text_encoder_out1,text_encoder_out1)
#                 attention,_ = self.multiheadAttention(mixup,audio_encoder_out,audio_encoder_out)
#                 # attention,_ = self.multiheadAttention(mixup,mixup,mixup)  #自注意力机制
#                 mixup = self.layernorm(residual + attention)
                
#                 mix_encoder_out = st_enc_out
#                 mix_encoder_out["encoder_out"][0] = mixup.transpose(0, 1)
#                 mix_loss, mix_output,mix_encoder_out,mix_padding_mask = self.forward_decoder(model, sample, mix_encoder_out, reduce)
...
#                 # kl_s_t = 0.5 * (self.compute_kl_loss(st_lprobs,mt_probs) + self.compute_kl_loss(mt_lprobs,st_probs))
#                 # contrastive_loss_st_mix = self.compute_contrastive_loss(mix_encoder_out,audio_encoder_out,reduce=True)
#                 # contrastive_loss_mt_mix = self.compute_contrastive_loss(mix_encoder_out,text_encoder_out,reduce=True)
#                 # contrastive_loss1 = contrastive_loss_st_mix + contrastive_loss_mt_mix
#                 # contrastive_loss = (contrastive_loss_st_mix / contrastive_loss1) * contrastive_loss_st_mix + (contrastive_loss_mt_mix / contrastive_loss1) * contrastive_loss_mt_mix
#                 if self.kl_st and self.kl_mt:
#                     kl_loss = kl_s + kl_t
#                     # kl_loss = kl_loss + (kl_s / kl_loss_1) * kl_s + (kl_t / kl_loss_1) * kl_t + (kl_s_t / kl_loss_1) * kl_s_t   #增加了一个KL散度kl_s_t
#                     # kl_loss = kl_loss + kl_s 
                
#                 # if self.kl_mt:
#                     # kl_loss = kl_loss + kl_t
#                 # loss = loss + self.kl_weight * kl_loss + 1.5 * contrastive_loss
#                 loss = loss + self.kl_weight * kl_loss
#                 # loss = (cross_loss / total_loss)*total_loss + (kl_loss / total_loss) * kl_loss + (contrastive_loss / total_loss) * contrastive_loss
#                 if self.ot_loss:
#                     ot_loss = ot_st_loss
...
#     #     else:
#     #         encoder_out = packed_encoder_out['encoder_out'][0]
#     #         encoder_padding_mask = packed_encoder_out['encoder_padding_mask'][0]
#     #         # short_audio_len = packed_encoder_out[]
...
#     def compute_contrastive_loss(self, mix_out, text_audio_output,reduce=True):
#         if mix_out.shape[1] != text_audio_output.shape[1]:
#             text_embedding_resized = F.interpolate(text_audio_output.transpose(1, 2), size=mix_out.size(1), mode='nearest').transpose(1, 2)
#             logits = F.cosine_similarity(mix_out,
#                                      text_embedding_resized,dim=-1)
#         else:
#             logits = F.cosine_similarity(mix_out,
#                                      text_audio_output,dim=-1)
...
#         probs = model.get_normalized_probs(net_output, log_probs=False)   #获取概率分布
#         lprobs = probs.log()
