...
#                 padding=k // 2,
#             )
#             for i,k in enumerate(kernel_sizes3)
#         )

#         self.crossatten = nn.MultiheadAttention(out_channels,num_heads=4,batch_first=True,dropout=0.1)
...
#         bsz, in_seq_len, _ = src_tokens.size()  # B x T x (C x D)  (32,160,768)
#         x3 = src_tokens.transpose(1, 2).contiguous()  # -> B x (C x D) x T (32,768,160)
#         x5 = src_tokens.transpose(1, 2).contiguous()
#         x7 = src_tokens.transpose(1, 2).contiguous()
#         layer_conv3 = []
#         for conv in self.conv_layers3:
#             x3 = conv(x3)
#             x3 = nn.functional.glu(x3,dim=1)
#             layer_conv3.append(x3)

#         layer_conv5 = []
#         for conv in self.conv_layers5:
#             x5 = conv(x5)    #第一个for循环(32,768,80)  (32,512,80)。第二个for循环 (32,1024,40),(32,512,40)
#             x5 = nn.functional.glu(x5, dim=1)
#             layer_conv5.append(x5)
        
#         m = nn.AdaptiveAvgPool1d(layer_conv5[1].size(-1))
#         layer_conv3[0] = m(layer_conv3[0])
#         layer_conv5[0] = m(layer_conv5[0])

#         x3 = layer_conv3[0] + layer_conv3[1]
#         x5 = layer_conv5[0] + layer_conv5[1]
#         x,_ = self.crossatten(x5.transpose(1,2),x3.transpose(1,2),x3.transpose(1,2))
...
class Conv1dSubsampler(nn.Module):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # m1 = nn.AdaptiveAvgPool1d(last_layer_output.shape[0])
        # res_first_layer_output = m1(first_layer_output.permute(1,2,0)).permute(2,0,1)
        # last_layer_output = last_layer_output + res_first_layer_output
        ...
        #动态加权和替代cross-attenion
        weights = []
        for layer_out in layer_outputs:
            avg_pooled = torch.mean(layer_out,dim=-1)
            weight = self.mlp(avg_pooled)
            weights.append(weight)
        
        weights = torch.cat(weights,dim=-1)
        weights = torch.softmax(weights,dim=-1)
        m1 = nn.AdaptiveAvgPool1d(layer_outputs[1].shape[-1])
        layer_outputs[0] = m1(layer_outputs[0])
        dynamic_sum_output = sum(weight.unsqueeze(-1) * layer_out for weight,layer_out in zip(weights.split(1,dim=-1),layer_outputs)) 
        refined_last_layer = self.layer_norm(layer_outputs[1].transpose(1,2) + dynamic_sum_output.transpose(1,2))
        layer_outputs[-1] = refined_last_layer.transpose(1,2)
