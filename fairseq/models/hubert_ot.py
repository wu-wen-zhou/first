...
class HuBERTOTEncoder(FairseqEncoder):
    ...
    def _forward(self, src_tokens, src_lengths, mode="st", return_all_hiddens=False):
        if mode == "st":
            ...
            src_tokens1 = src_tokens.cpu().to(dtype=torch.float32)
            fbanks = []
            for i in range(src_tokens.shape[0]):
                fbank = extract_fbank_features(src_tokens1[i].unsqueeze(0),16000)
                fbanks.append(fbank)
            fbanks = torch.tensor(fbanks).to("cuda" if torch.cuda.is_available() else "cpu")
